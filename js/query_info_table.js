...
function Query_Info_Table_Object2(options){
	...
	this.add_data_from_queries = add_data_from_queries;
	this.add_data_from_queries2 = add_data_from_queries2;
	this.hide = hide;
	this.show = show;
	this.expand = expand;
	this.collapse = collapse;
	...
	function add_data_from_queries2(iname,type){
		// integrate results from a number of queries and consolidate them into one 
		// list.  Queries should be passed as a set of sig_ids

		// set up varaibles for use later on
		var siginfo = 'http://api.lincscloud.org/a2/siginfo?callback=?';
		var tailinfo = 'http://api.lincscloud.org/a2/tailinfo?callback=?';
		...
		// make a RESTful call to tailinfo and parse the returned response into
		...
		function(sig_info_response){
			...
			sig_info_response.forEach(function(element,index,array){
				sigs.push(element.sig_id);
			});
			var sigs_string = '["' + sigs.join('","') + '"]';
			...
			function(response){
					if (response == 0){
						...
						response.forEach(function(element,index,array){
							for (var i=0; i < element.pert_iname.length; i++){
								if (element.pert_iname[i] !== null){
									query_result_data.push({pert_desc: element.pert_iname[i],
															cell_id: element.cell_id[i],
															pert_type: element.pert_type[i],
															score: element.score[i].toFixed(2)
															});
									self.autocomplete_data.push(element.pert_iname[i]);
									self.cell_ids.push(element.cell_id[i]);
									self.pert_types.push(element.pert_type[i]);
								}
							}
						});

						// convert the query_result_object to an array and sort it by score
						query_result_data.sort(function(a, b){
							return b.score-a.score;
						});

						//replace the data in the table
						self.collapse_panel.panels[0].grid_data = query_result_data;
						self.collapse_panel.panels[0].grid.setData(query_result_data);
						self.collapse_panel.panels[0].grid.render();
						self.show();
